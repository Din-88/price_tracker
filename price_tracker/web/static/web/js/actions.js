    ...
    .then((data) => {
        if ('error' in data){
            if(data.error.data.error === 'url unknown'){
                show_alert(
                    'В настоящий момент мы не можем обработать данную ссылку. ' +
                    'Сообщение об этом досадном случае уже отправлено администраторам, ' +
                    'и если будет техническая возможность, в ближайшее время мы это исправим.',
                    'new-trecker-alert')
            }
            return;
        }   
